...
if user_input:
    ...
    responses = [
        ...
        f"♻️ {st.session_state.username}, try reusing cardboard or bottles to make eco-friendly DIY projects 🌿",
        f"🌼 Remember to separate wet and dry waste, {st.session_state.username}, it makes composting easier 🌻",
        f"🍃 Using vegetable-based ink pens is an awesome way to stay sustainable, {st.session_state.username}! 🖊️",
        f"💚 Each time you recycle, {st.session_state.username}, you make our planet a little greener 🌏",
        f"🌺 Small eco choices matter, {st.session_state.username}! Switch off unused lights and plant a tree 🌳"
    ]
